...
module.exports = welcome = async (Fix, anj, jid, mek, key) => {
		...
		try {
			...
			if (anj.action == 'add') {
				...
				try {
					ppimg = await Fix.getProfilePicture(`${num.split('@')[0]}@c.us`)
				} catch {
...
try {
  ...
  metdata = await Fix.groupMetadata(anj.jid)
    if(anj.announce == 'false'){
    teks = `- [ Group Opened ] -\n\n_Group telah dibuka oleh admin_\n_Sekarang semua member bisa mengirim pesan_`
    Fix.sendMessage(metdata.id, teks, MessageType.text, {quoted: fkontakk})
    ...
    Fix.sendMessage(metdata.id, teks, MessageType.text, {contextInfo: {"mentionedJid": [tag]}, quoted: fkontakk})
